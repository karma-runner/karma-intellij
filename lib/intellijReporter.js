...
function FileListManager(globalEmitter) {
  ...
  this.dumpFiles = function() {
    if (currentFilesPromise) {
      currentFilesPromise.then(function(files) {
        process.stdout.write('##intellij-event[served-files:' + JSON.stringify(files.served) + ']\n');
...
function IntellijReporter(formatError, globalEmitter) {
  var fileListManager = new FileListManager(globalEmitter);
